...
public class DemoApp {
	...
	public static void main(String[] args) {
		...
		LogEntryManager lem = new LogEntryManager();
		for (int i = 0; i < 100; i++) {
			lem.add(createTestLogEntry());
		}
		System.out.println(lem.getLogEntries());

		FilterAnalyzer fa = new FilterAnalyzer("debug");
		fa.setLogEntries(lem.getLogEntries());
		fa.analyze();
		System.out.println("Filteranalyzer: " + fa.getResult());

		CountryAnalyzer ca = new CountryAnalyzer();
		ca.setLogEntries(lem.getLogEntries());
		ca.analyze();
		System.out.println("Countryanalyzer: " + ca.getResult());

		MaxLogEntryAnalyzer mlea = new MaxLogEntryAnalyzer();
		mlea.setLogEntries(lem.getLogEntries());
		mlea.analyze();
		System.out.println("MaxLogEntryAnalyzer: " + mlea.getLogEntry());

		UniqueAnalyzer ua = new UniqueAnalyzer();
		ua.setLogEntries(lem.getLogEntries());
		ua.analyze();
		System.out.println("UniqueAnalyzer " + ua.getResult());
	}
